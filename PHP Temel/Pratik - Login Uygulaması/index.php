...
if(file_exists('db/'.session('kullanıci_adi').'.txt')){
    $hakkimda=file_get_contents('db/'.session('kullanıci_adi').'.txt');
}else{
    $hakkimda='';
}
...
<div  class="container d-flex align-items-center justify-content-center">
    <div class="<? cookie('color') ? cookie('color') : 'bg-dark'; ?>" style="width: 18rem;">
        <div class="card-header bg-primary">
            Profilim
        </div>
        <div class="card-body">
            <h5 class="card-title text-warning"><?=  session('fullname') ?></h5>
